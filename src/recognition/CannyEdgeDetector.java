...
public class CannyEdgeDetector {
	...
	public CannyEdgeDetector( Context sketchy) {
		
		int imageWidth = 50;
		int baseImageWidth = 50;
		
	    SharedPreferences prefs=PreferenceManager.getDefaultSharedPreferences(sketchy);

	   	String lowThresholdS = prefs.getString("lowThreshold", "2.5");  // real default is set in Settings
	  	String highThresholdS = prefs.getString("highThreshold", "7.5");
	  	String gaussianKernelRadiusS = prefs.getString("gaussianKernelRadius", "2");
	 	String gaussianKernelWidthS = prefs.getString("gaussianKernelWidth", "16");
	 	
	 	contrastNormalized = prefs.getBoolean("contrastNormalized", false);
	 	
	 	lowThreshold = Float.valueOf(lowThresholdS);
	 	highThreshold = Float.valueOf(highThresholdS);

	 	gaussianKernelRadius = (Float.valueOf(gaussianKernelRadiusS) * imageWidth) / baseImageWidth;
	 	gaussianKernelWidth = (Integer.valueOf(gaussianKernelWidthS) * imageWidth) / baseImageWidth;
	 	
	 	if( gaussianKernelRadius < 0.1 )
